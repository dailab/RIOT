/*
 * Copyright (C) 2014 Freie Universität Berlin
 *               2015 Kaspar Schleiser <kaspar@schleiser.de>
 *
 * This file is subject to the terms and conditions of the GNU Lesser
 * General Public License v2.1. See the file LICENSE in the top level
 * directory for more details.
 */

/**
 * @defgroup    drivers_cc1x0x CC110x
 * @ingroup     drivers_netdev
 * @brief       TI CC110x
 * @{
 * @file
 * @brief       Public interface for cc1x0x driver
 * @author      Kaspar Schleiser <kaspar@schleiser.de>
 */

#ifndef CC1X0X_H
#define CC1X0X_H

#ifdef __cplusplus
extern "C" {
#endif

#include "periph/spi.h"
#include "periph/gpio.h"
#include "cc1x0x-internal.h"
#include "net/gnrc/nettype.h"

/**
 * @brief Struct for holding cc1x0x IO parameters
 */
typedef struct cc1x0x_params {
    spi_t spi;          /**< what */
    gpio_t cs;          /**< does */
    gpio_t gdo0;        /**< this */
    gpio_t gdo1;        /**< look */
    gpio_t gdo2;        /**< like */
} cc1x0x_params_t;

/**
 * @brief forward declaration
 */
typedef struct cc1x0x cc1x0x_t;

/**
 * @brief Struct for holding cc1x0x device state
 */
struct cc1x0x {
    cc1x0x_params_t params;                     /**< cc1x0x IO configuration */

    cc1x0x_statistic_t cc1x0x_statistic;        /**< Statistic values for
                                                     debugging */

    uint8_t radio_state;                        /**< Radio state */
    uint8_t radio_channel;                      /**< current Radio channel */
    uint8_t radio_address;                      /**< current Radio address */

    cc1x0x_pkt_buf_t pkt_buf;                   /**< RX/TX buffer */
    void (*isr_cb)(cc1x0x_t *dev, void* arg);   /**< isr callback */
    void *isr_cb_arg;                           /**< isr callback argument */
#ifdef MODULE_GNRC_NETIF
    gnrc_nettype_t proto;                       /**< protocol the radio expects */
#endif
};

/**
 * @brief Setup cc1x0x device parameters
 *
 * @param[in] dev       device struct to set up
 * @param[in] params    struct holding parameters
 *
 * @return always succeeds
 */
int cc1x0x_setup(cc1x0x_t *dev, const cc1x0x_params_t *params);

/**
 * @brief Set cc1x0x channel number
 *
 * @param[in] dev       device to work on
 * @param[in] channr    guess what
 *
 * @return nr of set channel on success
 * @return -1 on error
 */
int16_t cc1x0x_set_channel(cc1x0x_t *dev, uint8_t channr);


/**
 * @brief Send raw cc1x0x packet
 *
 * @param[in] dev       Device to send on
 * @param[in] packet    ptr to packet to be sent
 *
 * @return size of packet on success
 * @return <0 on error
 */
int cc1x0x_send(cc1x0x_t *dev, cc1x0x_pkt_t *packet);

/**
 * @brief Set cc1x0x radio address
 *
 * @param[in] dev   device to query
 *
 * @return nr of currently set address
 */
uint8_t cc1x0x_get_address(cc1x0x_t *dev);

/**
 * @brief Set cc1x0x radio address
 *
 * @param[in] dev       device to work on
 * @param[in] address   new address
 *
 * @return address set on success
 * @return 0 on error
 */
uint8_t cc1x0x_set_address(cc1x0x_t *dev, uint8_t address);


/**
 * @brief Set cc1x0x monitor mode setting
 *
 * @param[in] dev   device to work on
 * @param[in] mode  mode to set (0 or 1)
 */
void cc1x0x_set_monitor(cc1x0x_t *dev, uint8_t mode);

#ifdef __cplusplus
}
#endif

#endif /* CC1X0X_H */
/** @} */
